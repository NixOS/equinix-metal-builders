  ...
  systemd.services.add-disks-to-swap = {
    ...
    script = ''
      ...
      if [ ! -e /dev/md/spill.decrypted ]; then
        ...
        ${pkgs.coreutils}/bin/cat disklist | ${pkgs.findutils}/bin/xargs ${pkgs.mdadm}/bin/mdadm /dev/md/spill.decrypted --create --level=0 --run --force --raid-devices=$(${pkgs.coreutils}/bin/cat disklist | ${pkgs.busybox}/bin/wc -l)
        rm disklist
      fi
